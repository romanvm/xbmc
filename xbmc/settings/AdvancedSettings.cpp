    ...
    while (pRegExp)
    {
      if (pRegExp->FirstChild())
      {
        ...
        if (pRegExp->ToElement())
        {
          ...
          std::string byTitle = XMLUtils::GetAttribute(pRegExp->ToElement(), "bytitle");
          if (byTitle == "true")
          {
            bByTitle = true;
          }
        ...
        if (iAction == 2)
          settings.insert(settings.begin() + i++, 1, TVShowRegexp(bByDate, regExp, iDefaultSeason, bByTitle));
        else
          settings.push_back(TVShowRegexp(bByDate, regExp, iDefaultSeason, bByTitle));
      }
